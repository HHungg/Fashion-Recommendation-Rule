...
public class ProductData {
    ...
    public static String updateProduct(String pId, Products p) throws SQLException {
        ...
        try {
             ...
             DbConnect connect = new DbConnect();
             String query = "UPDATE Products SET PID='"+p.getPID()+"', PName= N'"+p.getPName()+"', InputPrice = '"+p.getInputPrice()+"' , PPrice = '"+p.getPPirce()+"',  Img = N'"+p.getImg()+"', SID = '"+p.getSID()+"', CategoryId = '"+p.getCategoryId()+"' , PDescription = '"+p.getPDescription()+"' WHERE (PID='"+pId+"')" ;
             PreparedStatement ps = (PreparedStatement) connect.con.prepareStatement(query);
